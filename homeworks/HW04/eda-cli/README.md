...
- вызывает функции из `eda_cli.core`:
  ...
  - `missing_table`,
  - `compute_quality_flags`;
- возвращает флаги из `compute_quality_flags`.
...
- `too_few_rows` - флаг, слишком мало строк;
- `too_many_columns` - флаг, слишком много колонок;
- `max_missing_share` - процент пропущеных значений;
- `too_many_missing` - флаг, слишком много пропусков;
- `has_constant_columns` - флаг, есть константная колонка;
- `has_suspicious_id_duplicates` - флаг, есть дублирующийся id;
- `quality_score` - итоговый счет;
